  ...
  const handleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (
        ...
        userData?.message === "USER IS STUDENT"
      ) {
        setIsInvalidCreds(false);
        ...
        <div className="bg-gray-100 max-w-[2050px] mx-auto animate-fadeUp">
          <main className="flex min-h-screen">
            <div className="  p-6 pt-2 flex flex-col w-full   gap-2 lg:gap-6 lg:flex-row rounded-lg px-4 justify-center items-center m-2 ">
              <Link
                href="/"
                className="flex items-center relative flex-col mb-2 lg:mb-10 flex-grow justify-center"
              >
                <Image
                  src={logo}
                  priority
                  className="w-[60%] md:w-auto max-w-[520px] max-h-[480px]  h-auto"
                  alt="logo"
                />
                <h2
                  style={{ textShadow: "2px 4px 3px rgba(44, 56, 74, 0.3)" }}
                  className="px-4 lg:px-0 font-black text-xl  text-center text-black"
                >
                  ASSISTANT-DRIVEN ATTENDANCE SYSTEM
                </h2>
              </Link>
              <div className="flex flex-col justify-center items-center bigscreens:items-start flex-grow w-full lg:w-auto">
                <div className="flex justify-center items-center w-full max-w-[400px] mx-auto lg:mx-4 2xl:mr-auto bigscreens:mx-0 bg-[#fff000] pt-2 flex-col shadow rounded-md drop-shadow">
                  <div className="flex flex-col gap-2 justify-center items-center">
                    <Image
                      src={studentassistant}
                      className="w-24 smallerscreens:w-32 rounded-md mx-2 lg:mx-0 h-auto "
                      alt=""
                    />
                    <h1 className="text-center pt-2 px-2  text-sm sm:text-lg font-bold">
                      LOGIN AS STUDENT ASSISTANT
                    </h1>
                  </div>
                  <form
                    className={`w-full px-6 lg:px-10 pt-6 pb-4 `}
                    onSubmit={handleLogin}
                  >
                    <div
                      className={` rounded-lg relative bg-white  ${
                        isUsernameEmpty ? "border-[#ff0000] mb-7" : "mb-4"
                      }`}
                    >
                      <input
                        value={username}
                      ...
                      <label
                        htmlFor="username"
                        className={`tracking-wider absolute text-xs lg:text-sm text-[#888] duration-150 transform -translate-y-4 scale-75 top-2 z-10 origin-[0] bg-white px-2 peer-focus:px-2  peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto start-1 rounded-lg ${
                          isUsernameEmpty
                            ? "text-[#ff0000]"
                            : "peer-focus:text-[#01579B]"
                        }`}
                      >
                        Username
                      </label>
                    ...
                    <div
                      className={` rounded-lg relative bg-white  ${
                        isPasswordEmpty ? "border-[#ff0000] mb-9" : "mb-4"
                      }`}
                    >
                      <input
                        value={password}
                      ...
                      <i
                        ...
                        className={`text-sm toggle-password fa-solid ${
                          passwordVisible ? "fa-eye" : "fa-eye-slash"
                        } absolute top-1/2 right-0 transform -translate-y-1/2 px-2 focus:outline-none cursor-pointer`}
                      ></i>
                      {isPasswordEmpty && (
                        <div className="items-center gap-1 flex flex-row ml-1 absolute top-12 left-0">
                          <i className="text-[#ff0000] fa-solid text-xs mt-[2px] fa-circle-exclamation"></i>
                    ...
                    {isLoginPressed ? (
                      <button
                        type="submit"
                        className="text-white  outline-none border-0 flex flex-wrap justify-center flex-row items-center gap-1 w-full py-3  h-auto shadow bg-[#01579B]  hover:border-transparent transition ease-in duration-200 transform hover:-translate-y-1 active:translate-y-0 rounded-md mt-5"
                      ...
                      <button
                        type="submit"
                        className="text-white outline-none border-0 block w-full py-3 text-sm font-bold h-auto shadow bg-[#01579B]  hover:border-transparent transition ease-in duration-200 transform hover:-translate-y-1 active:translate-y-0 rounded-md mt-5"
                        disabled={isLoginPressed}
                      >
                        LOGIN
                      </button>
                  ...
                  <span className="text-black font-semibold mb-4 mx-auto text-sm">
                    Faculty Admin?{" "}
                    <Link
                      className="font-bold text-[#01579B] underline"
                      href="/faculty-admin/login"
                    >
                      Click here
                    </Link>
                  </span>
