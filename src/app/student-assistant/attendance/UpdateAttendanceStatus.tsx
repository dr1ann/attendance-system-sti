...
const UpdateAttendanceStatus: React.FC<ModalProps> = ({ isVisible, currentAttendanceStatus, teacherName, studentName, subject, roomNum, scheduledDate, scheduledInTime, scheduledOutTime, setRefetch, onClose, scheduleId }) => {
    ...
    return (
        <div className="flex lg:flex-wrap items-start lg:items-center flex-col lg:justify-center mt-4 lg:mt-0 gap-3">
            <h3 className="text-sm text-center font-semibold">Choose Attendance Color:</h3>
            <div className="flex flex-row gap-6">
                {currentAttendanceStatus !== 'PRESENT' &&
                <AlertDialog key="present">
                    <AlertDialogTrigger className="cursor-pointer w-8 h-auto" onClick={() => setSelectedStatus('PRESENT')}>
                    ...
                    {selectedStatus === 'PRESENT' &&
                        <AlertDialogContent>
                            <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                    Are you sure you want to mark teacher {teacherName}'s attendance status as Present?
                                </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                                <AlertDialogCancel>No</AlertDialogCancel>
                                <AlertDialogAction
                                    className={`bg-[#2C384A] ${isSaveButtonPressed ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                ...
                <div className="flex animate-fadeUp z-[99999] min-h-screen m-auto overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 bottom-0 justify-center items-center w-full md:inset-0 max-h-full">
                    <div
                        id="alert-3"
                        className={`animate-fadeUp absolute bottom-0 flex items-center p-4 mb-4 ${isStatusUpdated ? 'text-[#28a745]' : 'text-[#ff0000]'} rounded-lg ${isStatusUpdated ? 'bg-green-50' : 'bg-red-50'}`}
                        role="alert"
                    >
                        <i className={`${isStatusUpdated ? 'text-[#28a745]' : 'text-[#ff0000]'} fa-solid ${isStatusUpdated ? 'fa-check' : 'fa-times'}`}></i>
                        <span className="sr-only">{isStatusUpdated ? 'Success' : 'Error'}</span>
                ...
                <AlertDialog key="late">
                    <AlertDialogTrigger className="cursor-pointer w-8 h-auto" onClick={() => setSelectedStatus('LATE')}>
                    ...
                    {selectedStatus === 'LATE' &&
                        <AlertDialogContent>
                            <AlertDialogHeader>
                                <AlertDialogTitle>Confirm Status Change</AlertDialogTitle>
                                <AlertDialogDescription>
                                Are you sure you want to mark teacher {teacherName}'s attendance status as Late?
                                </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                                <AlertDialogCancel>No</AlertDialogCancel>
            ...
            {currentAttendanceStatus !== 'ABSENT' &&
                <AlertDialog key="absent">
                    <AlertDialogTrigger className="cursor-pointer w-8 h-auto" onClick={() => setSelectedStatus('ABSENT')}>
                    ...
                    {selectedStatus === 'ABSENT' &&
                        <AlertDialogContent>
                            <AlertDialogHeader>
                                <AlertDialogTitle>Confirm Status Change</AlertDialogTitle>
                                <AlertDialogDescription>
                                Are you sure you want to mark teacher {teacherName}'s attendance status as Absent?
                                </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                                <AlertDialogCancel>No</AlertDialogCancel>
