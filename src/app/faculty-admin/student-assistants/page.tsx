...
const StudentAssistants = () => {
  ...
  const handleMoreInfoClick = (e:React.MouseEvent<HTMLButtonElement>, studentId: string) => {
    ...
    return (
      ...
      <label htmlFor="sort" className="text-xs font-medium whitespace-nowrap">Sort by:</label>
      <select
        id="sort"
        ...
        <option value="oldest">Oldest - Newest</option>
         ...
         ?
          <ul className="grid grid-cols-1 relative gap-6">
               {sortedStudents?.map((student:User, index:number) => (
              ...
              <li key={index} style={{border: "1px solid #D9D9D9"}} className="flex flex-wrap flex-col  gap-3 p-3 rounded-lg shadow drop-shadow">
                ...
                <div className="flex flex-row justify-between items-center gap-8">
                  ...
                  <button className=" shadow  bg-transparent  border-[1px] border-[#ff0000] border-dashed hover:text-white hover:bg-[#ff0000]  hover:border-transparent transition ease-in duration-200 transform hover:-translate-y-1 active:translate-y-0 rounded-lg px-2 py-1  cursor-pointer flex  justify-center flex-row items-center gap-1   ">
                      <i  className="fa-solid fa-user-minus text-xs text-center "></i>
                      <span className="text-sm text-center">Remove Account</span>
                  </button>
                  </div>
      ...
      <table className="table-auto rounded-lg w-fit mx-auto text-black text-left rtl:text-right">
          <thead className="bg-[#2C384A] shadow drop-shadow rounded-lg">
              <tr className=" shadow drop-shadow ">
                  <th scope="col" className="px-4 py-2 text-center text-base text-white shadow drop-shadow">NAME</th>
          ...
          {sortedStudents.map((student:User, index:number) => (
              <tr key={index}>
                  ...
                  <th scope="row" className=" px-4 py-2 font-normal text-base bg-gray-100 h-full  shadow drop-shadow  ">
                        ...
                        <div className="flex flex-col ">
                         <span className="text-xs  text-[#333333] ">Student ID: <br/></span> 
                          <h4 className="font-semibold text-sm ">{student?.studentId || 'N/A'}</h4>
                          </div>
                          <div className="flex flex-col ">
...
export default StudentAssistants