...
const ActivityLog: React.FC<ModalProps> = ({ isVisible, onClose, studentId, studentName }) => {
 ...
 const sortNewestToOldest = useMemo(() => {
    ...
    return (
        <div data-modal-backdrop="add" aria-hidden="true" className="flex animate-fadeUp z-[99999] min-h-screen backdrop-blur-sm m-auto overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 bottom-0 justify-center items-center w-full md:inset-0 max-h-full">
            ...
            <div className={`relative p-4 w-full ${isFetchSuccessful ? 'max-w-fit' : 'max-w-[46rem]'} m-auto`}>
                {/* Modal content */}
                <div className="relative bg-white rounded-lg shadow">
                    {/* Modal header */}
                    <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t gap-2">
                        <h3 className="text-lg lg:text-xl font-bold text-gray-900">
                            <i className="fa-solid fa-chart-line text-base lg:text-lg text-center text-[#2C384A]"></i> {`${studentName}'s Activity Log`}
                        </h3>
        ...
        <button
                      ...
                      >
                        ...
                        <span id="showAdd" className="font-semibold text-sm text-center">Export Activity Log</span>
                      </button>
        </div>
                                 <table className="w-full text-left rtl:text-right text-white">
                                 <thead>
                                   <tr className="bg-[#2C384A]">
                                     <th scope="col" id="activityHeader" className="text-center text-sm lg:text-base px-6 py-3">SUBJECT</th>
                                     <th scope="col" id="roomNumHeader" className="text-center text-sm lg:text-base px-6 py-3">ROOM NO.</th>
                                     <th scope="col" id="dateTimeHeader" className="text-center text-sm lg:text-base px-6 py-3">SCHEDULED DATE & TIME</th>
                                     <th scope="col" id="dateTimeHeader" className="text-center text-sm lg:text-base px-6 py-3">ACTIVITY</th>
                                     <th scope="col" id="dateTimeHeader" className="text-center text-sm lg:text-base px-6 py-3">UPDATED ON</th>
                                   </tr>
                                 </thead>
                                 <tbody>
                                   {sortedActivities?.map((activity: ActivityLog) => (
                                     <tr key={activity?.id}>
                                       <td scope="row" className="px-6 py-3 text-center text-sm  text-black lg:text-base bg-gray-100 h-full shadow drop-shadow">
                                       ...
                                       <td className="px-6 py-3 text-center text-sm lg:text-base bg-gray-100 text-black h-full shadow drop-shadow">
                                         ...
                                         <br />
                                         ({formatTime(activity?.scheduledInTime) || 'N/A'} - {formatTime(activity?.scheduledOutTime) || 'N/A'})
                                       </td>
