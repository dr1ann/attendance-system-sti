...
const AddNewStudent: React.FC<ModalProps> = ({
  setRefetch,
  isVisible,
  onClose,
}) => {
  ...
  const generateRandomUsername = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    setNote(
      "Note: Please copy the generated username and password and give it to the student ASAP for them to change it."
    );
  };
    ...
    <>
      ...
      >
        {!isSuccessful ? (
            ...
            <div className="relative bg-white rounded-lg shadow ">
              ...
              <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t gap-2">
                <h3 className="text-lg lg:text-xl font-bold text-gray-900 ">
                  <i className="fa-solid fa-user-plus text-base lg:text-lg text-center text-[#2C384A]"></i>{" "}
              ...
              {/* <!-- Modal body --> */}
              <form
                className="pb-4 px-4 md:pb-5 md:px-5 gap-4"
                onSubmit={handleAddStudent}
              >
                <div
                  className={`grid grid-cols-2 gap-4 mt-3 ${
                    isNameEmpty || isYearLevelEmpty
                      ? "mb-[45px] smallerscreens:mb-[35px] lg:mb-[30px]"
                      : ""
                  }`}
                >
                    ...
                    <div className="flex flex-row flex-wrap items-center absolute top-1/2 right-0 transform -translate-y-1/2">
                      {newStudent?.username && (
                        <button
                          ...
                          className="text-[#2C384A] animate-fadeIn shadow bg-transparent border-[1px] hover:bg-[#D9D9D9] hover:border-transparent transition ease-in duration-200 active:translate-y-0 text-[10px] lg:text-xs rounded-lg px-2 py-1 mr-2 font-semibold"
                        >
                          {copyUsernameSuccess ? (
                      ...
                      <button
                        ...
                        className="shadow bg-transparent border-[1px] hover:bg-[#D9D9D9] hover:border-transparent transition ease-in duration-200 active:translate-y-0 text-[10px] lg:text-xs rounded-lg px-2 py-1 mr-2 font-semibold"
                      >
                        Generate
                      </button>
                    </div>
                    ...
                    <div className="flex flex-row flex-wrap items-center absolute top-1/2 right-0 transform -translate-y-1/2">
                      {newStudent?.password && (
                        <button
                          ...
                          className="text-[##2C384A] animate-fadeIn shadow bg-transparent border-[1px] border-[#D9D9D9] hover:bg-[#D9D9D9] hover:border-transparent transition ease-in duration-200 active:translate-y-0 text-[10px] lg:text-xs rounded-lg px-2 py-1 mr-2 font-semibold"
                        >
                          {copyPasswordSuccess ? (
                            <i className="text-[#28a745] transition-opacity duration-500 ease-in-out fa-solid fa-check animate-fadeIn"></i>
                          ) : (
                      ...
                      <button
                        ...
                        className="shadow bg-transparent border-[1px] border-[#D9D9D9] hover:bg-[#D9D9D9] hover:border-transparent transition ease-in duration-200 active:translate-y-0 text-[10px] lg:text-xs rounded-lg px-2 py-1 mr-2 font-semibold"
                      >
                        Generate
                      </button>
                    </div>
                ...
                <div
                  className={`grid grid-cols-2 gap-4 mt-6 ${
                    isAcademicLevelEmpty || isSectionEmpty
                      ? "mb-[45px] smallscreens:mb-[35px] lg:mb-[30px]"
                      : ""
                  }`}
                >
                  <div className="rounded-lg relative">
                ...
                <div
                  className={`grid grid-cols-2 gap-4 mt-6 ${
                    isProgramEmpty || isStudentIdEmpty || studentIdError
                      ? "mb-[45px] smallerscreens:mb-[35px] lg:mb-[30px]"
                      : ""
                  }`}
                >
                  ...
                  <div
                    ...
                    className={`${
                      isGenderEmpty
                        ? "border-[#ff0000]"
                        : "focus:border-[#01579B]"
                    } border-[1px] rounded-lg relative flex flex-row px-3 justify-start items-center`}
                  >
                    {/* <!-- Gender label --> */}
                    <span
                      className={`${
                        isGenderEmpty
                          ? "text-[#ff0000]"
                          : "text-[#888] peer-focus:text-[#01579B]"
                      } tracking-wide absolute text-sm duration-150 transform -translate-y-4 scale-75 top-2 z-10 origin-[0] bg-white px-2 peer-focus:px-2 peer-focus:text-[#01579B] peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto start-1`}
                    >
                      Sex
                    </span>
                    ...
                    <div className="flex items-center flex-row justify-center p-4 flex-wrap space-x-4">
                      ...
                      <div className="flex items-center gap-1">
                        <input
                          onChange={handleInputChange}
                          type="radio"
                          className="form-radio text-[#01579B]"
                          name="gender"
                          id="male"
                          value="Male"
                        />
                        <label
                          htmlFor="male"
                          className="text-sm lg:text-base text-[#2C384A] text-center"
                        >
                          Male
                        </label>
                      </div>
                ...
                {isAddBtnPressed ? (
                  <button
                    type="submit"
                    className="flex flex-row px-5 cursor-not-allowed shadow  bg-transparent  border-[1px] border-[#D9D9D9]    hover:border-transparent transition ease-in duration-200 transform hover:-translate-y-1 active:translate-y-0 items-center mx-auto w-fit my-5 gap-1 py-2 rounded-lg"
                    disabled={isAddBtnPressed}
                  >
                  ...
                  <button
                    type="submit"
                    className="flex flex-row px-5 cursor-pointer shadow  bg-transparent  border-[1px] border-[#D9D9D9]  hover:bg-[#D9D9D9]  hover:border-transparent transition ease-in duration-200 transform hover:-translate-y-1 active:translate-y-0 items-center mx-auto w-fit my-5 gap-1 py-3 rounded-lg"
                    disabled={isAddBtnPressed}
                  >
          ...
          <div
            id="alert-3"
            className="animate-fadeUp absolute bottom-0 flex items-center p-4 mb-4 text-[#28a745] rounded-lg bg-green-50  "
            role="alert"
          >
